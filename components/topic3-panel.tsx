...
import { ProjectMilestone, ProjectStatus, Topic3Progress } from "@/types/domain";
import { useMemo, useState } from "react";

type Props = {
  projectId: string;
  initialMilestones: ProjectMilestone[];
  initialProgress: Topic3Progress;
};
...
export function Topic3Panel({ projectId, initialMilestones, initialProgress }: Props) {
  const [milestones, setMilestones] = useState<ProjectMilestone[]>(initialMilestones);
  const [progress, setProgress] = useState<Topic3Progress>(initialProgress);
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(`/api/projects/${projectId}/topic3/milestones/${selectedMilestone.id}`, {
      method: "PATCH",
    ...
    if (body.milestone) setMilestones((prev) => prev.map((m) => (m.id === body.milestone.id ? body.milestone : m)));
    if (body.topic3Progress) setProgress(body.topic3Progress);
  ...
  return (
    <section className="card grid">
      <h2>פרק 3 - מכרזים</h2>
      <p>מצב פרק 3: <strong>{progress.isComplete ? "הושלם" : "בתהליך"}</strong></p>
      <ul>
        {progress.domains.map((d) => (
          <li key={d.domainIndex}>{d.domainName}: {d.isComplete ? "הושלם" : "פתוח"} {d.isRelevant ? "" : "(לא רלוונטי)"}</li>
        ))}
      </ul>
      ...
      {selectedMilestone ? (
        <form className="grid" onSubmit={onSubmit}>
          <label>
            <div className="field-label">שלב לעדכון</div>
          ...
          <button type="submit" className="menu-toggle" style={{ display: "inline-flex" }}>שמירת שלב</button>
