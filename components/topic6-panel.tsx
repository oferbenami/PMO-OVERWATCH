...
import { ProjectMilestone, ProjectStatus, Topic6Progress } from "@/types/domain";
import { useMemo, useState } from "react";

type Props = {
  projectId: string;
  initialMilestones: ProjectMilestone[];
  initialProgress: Topic6Progress;
};
...
export function Topic6Panel({ projectId, initialMilestones, initialProgress }: Props) {
  const [milestones, setMilestones] = useState<ProjectMilestone[]>(initialMilestones);
  const [progress, setProgress] = useState<Topic6Progress>(initialProgress);
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(`/api/projects/${projectId}/topic6/milestones/${selectedMilestone.id}`, {
      method: "PATCH",
    ...
    if (body.topic6Progress) setProgress(body.topic6Progress);
    setMessage("נשמר בהצלחה");
  };

  return (
    <section className="card grid">
      <h2>פרק 6 - אחרי האכלוס</h2>
      <p>
        מצב מעקב פרק 6: <strong>{progress.isTrackingComplete ? "הושלם" : "בתהליך"}</strong>
      </p>
      <ul>
        {progress.subtopics.map((s) => (
          <li key={s.subtopicIndex}>
            {s.subtopicName}: {s.isComplete ? "הושלם" : "פתוח"} {s.isRequired ? "" : "(לא נדרש)"}
          </li>
        ))}
      </ul>
